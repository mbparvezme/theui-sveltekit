<script lang="ts">
  ...
  let ctx: { btnClass?: string; iconClass?: string } = getContext(FAB) as { btnClass?: string; iconClass?: string } || {};
  let elem: HTMLElement | null;

  onMount(() => {
    elem = document.getElementById(id);
    const toggleOpen = () => elem?.classList.toggle("open");
    elem?.addEventListener("click", toggleOpen);

    return () => {
      elem?.removeEventListener("click", toggleOpen);
    }
  });

  const fabButtonClass = () => twMerge(ctx.btnClass, $$props.class);
  const fabIconClass = () => twMerge(ctx.iconClass, iconClass);
</script>

<svelte:element {id} this={href ? "a" : "button"} {href}
  class={fabButtonClass()}
  role={href ? "link" : "button"}
  aria-label={href ? "link" : "button"}
