<script lang="ts">
  ...
  let C: any = {animate: animationSpeed, labelClasses, rounded, size, reset: !!$$restProps?.reset};
...
<label  for={id}
  ...
  <input class={getInputClasses(C, $$restProps, "checkbox")} {...$$restProps} class:sr-only={$$restProps?.custom} {id} {name} type="checkbox" bind:value>
  {#if $$restProps?.custom}
    <slot />
  {:else}
    {@html label}
