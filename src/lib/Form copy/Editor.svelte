<script lang="ts">
  const tools = ["blockquote", "code", "codeblock", "link", "horizontalLine", "youtube", "highlighter", "color", "redoUndo"] as const
  ...
  export let miniEditor: boolean = false
  export let exclude: Tools[] = []

  const ctx: any = getContext( FORM || {} )
  let C:INPUT_CONFIG = {rounded : "md", grow: true, size: "md", variant: "bordered" }
...
<div class={getInputContainerClass(C)}>
  {#if editor}
    ...
    <div class="flex flex-wrap editor-toolbar gap-1">
      ...
      <Dropdown animation="fade" closeOnOutsideClick={false} size="custom" style="--dropdown-width: 148px" >
      ...
      {#if !miniEditor && (!exclude.includes("blockquote") && !exclude.includes("code") && !exclude.includes("codeblock"))}
      <div class="editor-btn-group">
        {#if !exclude.includes("blockquote")}
      ...
      {/if}
      {#if !miniEditor && (!exclude.includes("link") && !exclude.includes("horizontalLine") && !exclude.includes("youtube"))}
      <div class="editor-btn-group">
        ...
        {/if}
        {#if !exclude.includes("horizontalLine")}
      ...
      {#if !miniEditor && (!exclude.includes("highlighter") && !exclude.includes("color"))}
      ...
      {/if}
      {#if !exclude.includes("highlighter")}
      <Dropdown animation="fade" closeOnOutsideClick={false} size="custom" style="--dropdown-width: 116px" >
      ...
      {#if !miniEditor || !exclude.includes("redoUndo")}
      <div class="editor-btn-group">
