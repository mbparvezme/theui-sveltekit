<script lang="ts">
  ...
  let getClasses  = () => "flex items-center w-max gap-4 cursor-pointer pe-4 " + (fill ? "text-gray-500 bg-tertiary font-semibold text-sm" : "border border-gray-100") + getRounded(round)
...
<span class={"theui-chips " + twMerge(getClasses(), $$props?.class)} class:ps-4={!imgUrl} class:py-2={!imgUrl} bind:this={nodeRef}>
