<script lang="ts">
  ...
  let getClasses  = () => `flex items-center w-max gap-4 cursor-pointer pr-4 ${(fill ? "text-gray-500 bg-tertiary font-semibold text-sm" : "border border-gray-100")} ${imgUrl?"":"pl-4 py-2"}${getRounded(round)}`;
...
<span class={`theui-chips ${twMerge(getClasses(), $$props?.class)}`} bind:this={nodeRef}>
