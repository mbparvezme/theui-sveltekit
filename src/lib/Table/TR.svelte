<script lang="ts">
  ...
  const { config } = getContext(TABLE) as any;

  export let data : TABLE_ROW	= undefined;
  export let keys : string[]|undefined = undefined;
  export let headingRow : boolean = false;

  let getRowClass = () => `${((!headingRow && (config?.border == "both" || config?.border == "y")) ? "border-y " : "")} ${((config?.hover && !headingRow) ? `${getAnimate(config?.animate)} hover:bg-gray-100 dark:hover:bg-gray-800 ` : "")} ${(typeof config?.stripe == "string" ? config?.stripe : config?.stripe === true ? " even:bg-gray-50 dark:even:bg-gray-800/50 " : "")} ${config?.borderColor} ${config?.trClass||""}`
