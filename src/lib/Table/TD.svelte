<script lang="ts">
  ...
  let getCellClass = () => `${(config?.border == "both" || config?.border == "x") ? `${config?.borderColor} border-l border-r ` : " "} ${sizeCls(config?.space || "default")} ${tag=="td" ? " text-gray-600 dark:text-gray-400 font-normal" : " font-medium text-sm"}`;
