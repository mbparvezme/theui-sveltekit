...
{#if ST_TABS.selectedPanel === id}
	{#if CTX.animate}
		<div class="theui-tab-panel {twMerge(CTX.tabPanelClasses, props?.class as string)}" in:fade={{duration:150}}>
      {@render children?.()}
    </div>
	{:else}
		<div class="theui-tab-panel {twMerge(CTX.tabPanelClasses, props?.class as string)}">
      {@render children?.()}
    </div>
	{/if}
{/if}
