...
// Input Functions
export let getInputContainerClass = (
  config: INPUT_CONFIG,
  type: string = "input",
): string => {
  ...
  let cls =
    ...
    (config?.reset
      ? "theui-reset "
      : "flex flex-col gap-2 " + (config?.grow ? "flex-grow " : ""));
...
export let getInputBoxClass = (
  config: INPUT_CONFIG,
  attr: any,
  type: string = "input",
) => {
  let cls =
    "theui-input-box relative inline-flex overflow-hidden " +
    (config?.variant == "flat"
      ? getRounded("none")
      : getRounded(config.rounded));
  cls +=
    config.variant == "filled"
      ? " bg-gray-50 dark:bg-gray-800 border border-gray-50 dark:border-gray-800"
      : config.variant == "bordered"
        ? " border border-gray-100 dark:border-gray-700 bg-transparent"
        : config.variant == "flat"
          ? type !== "file"
            ? " border-b-2 border-gray-100 dark:border-gray-700 bg-transparent"
            : ""
          : "";
...
export let getFormControlStyle = (
  config: INPUT_CONFIG,
  restAttr: any,
  propsClass: string,
  type: string = "input",
...
export let getInputSize = (
  size: "none" | "sm" | "md" | "lg" | "xl" | undefined,
  type: string = "input",
): string => {
  if (type == "file") {
    return size == "sm"
      ? " input-sm file:px-4 file:py-2 file:text-sm"
      : size == "md"
        ? " input-md file:px-6 file:py-3"
        : size == "lg"
          ? " input-lg file:px-6 file:py-4 file:text-xl"
          : size == "xl"
            ? " input-xl file:px-8 file:py-5 file:text-2xl"
            : "";
  }
...
export let inputClass = (
  ...
  let cls =
    "text-default border-transparent " +
    (type == "input" ? "bg-transparent" : "bg-secondary");
  cls +=
    getAnimate(config?.animate) +
    " focus:ring-brand focus:border-brand w-full ";
...
export let fileInputCls = (config: INPUT_CONFIG, attr: any): string => {
  ...
  let cls =
    config.variant == "filled"
      ? "bg-gray-50 dark:bg-gray-800 text-default border border-gray-50 dark:border-gray-800 focus:border-brand"
      : config.variant == "bordered"
        ? "border border-gray-100 dark:border-gray-700 text-default focus:border-brand bg-transparent focus:ring-brand"
        : config.variant == "flat"
          ? "border-0 text-default"
          : "";
  cls +=
    getAnimate(config?.animate) +
    " focus:ring-brand focus:border-brand focus:outline-brand text-gray-500/70 " +
    getRoundedFileBtn(config?.rounded);

  if (attr?.disabled)
    cls +=
      " disabled:cursor-not-allowed disabled:opacity-50 disabled:select-none";
  if (attr?.readonly)
    cls += " read-only:pointer-events-none read-only:opacity-50";

  return (
    cls +
    " file:mr-4 file:bg-brand/5 file:text-brand file:cursor-pointer cursor-pointer file:border-0"
  );
};

export let checkboxRadioClass = (
  config: INPUT_CONFIG,
  ...
  let cls =
    type +
    getRounded(type == "radio" ? "full" : config?.rounded) +
    " bg-gray-50 dark:bg-gray-800 border border-gray-400 dark:border-transparent text-brand focus-within:ring-brand dark:checked:bg-brand !ring-offset-primary peer";
  return cls;
};
