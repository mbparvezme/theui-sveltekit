...
<script lang="ts">
  ...
  let navClass = () => "bg-primary left-0 top-0 w-full flex items-center justify-center" + getAnimate(animationSpeed) + (miniNav || (hideNav === false && scrollPos !== 0) ? scrollClass : "");
  let navInnerClass = () => "nav-inner w-full max-w-[var(--max-width)] flex grow gap-x-8 items-center justify-between relative" + heightCls() + getAnimate(animationSpeed) + (miniNav || (hideNav === false && scrollPos !== 0) ? " px-4" : " px-8");
</script>
